package io.github.treesitter.jtreesitter.mlscript;

import java.lang.foreign.*;

public final class TreeSitterMlscript {
    ...
    private static final TreeSitterMlscript INSTANCE = new TreeSitterMlscript();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterMlscript() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_mlscript");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-mlscript");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-mlscript");
